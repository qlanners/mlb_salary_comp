...
for item in df.concat_cols:
    ...
    for s in all_p:
        if 'Drafted' in s.text and 'Player signed' not in s.text and 'did not sign' in s.text:
            continue

        elif 'Drafted' in s.text and 'Player signed' not in s.text and 'did not sign' not in s.text:
